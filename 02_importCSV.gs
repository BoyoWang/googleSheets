...
function Make_list_For_all_sheets(sheet, firstCellAddress_In_A1Style){
  ...
  var finalList = makeFirst2ArrayOfLists(address_firstCell_A1_Style.sheetListTitleColIndex, 
                                    address_firstCell_A1_Style.sheetList_MainTitleText)//First 2 rows
  ...
  sheet.getRange(sheetListFirstRange.getRow(), 
                 sheetListFirstRange.getColumn(), 
                 finalList.length, 
                 finalList[0].length).setValues(finalList);

  
  Logger.log("sheetList is made in sheet '" + sheet.getName() + "' on cell '" + firstCellAddress_In_A1Style + "'.");
...
function list_all_files_inside_folder(folderID, sheet, cellAddress_In_A1Style){
  ...
  //First 2 rows
  var list = makeFirst2ArrayOfLists(address_firstCell_A1_Style.csvFileListTitleColIndex, 
  ...
  sheet.getRange(firstCell.getRow() ,
                 firstCell.getColumn(), 
                 list.length, 
                 list[0].length).setValues(list);
  
  Logger.log("csvFileList is made in sheet '" + sheet.getName() + "' on cell '" + cellAddress_In_A1Style + "'.");
};
...
function importCSVFromGoogleDrive(fileID) {
  ...
  Logger.log("File '"+ file.getName() + "' is imported to sheet '" + nameExcludeExtension + "'");
}
...
function importAll_CSV_Files(){
  ...
  var rangeCSV_FileList = returnListRangeExcludeTopRows(mainSheet, address_firstCell_A1_Style.csvFileList, 2);  
  var rangeCSV_FileListData = rangeCSV_FileList.getValues();
  
  //Read col index for googleDriveID column
  var fileID_ColIndex = address_firstCell_A1_Style.csvFileListTitleColIndex.googleDriveID[0];
...
function Sort_CSV_FileList() {
  var spreadsheet = SpreadsheetApp.getActive();
  var mainSheet = spreadsheet.getSheetByName(name_importantSheets.mainSheet); 
  var rangeToSort = returnListRangeExcludeTopRows(mainSheet, address_firstCell_A1_Style.csvFileList, 2);  
  
  var columnToSort = address_firstCell_A1_Style.csvFileListTitleColIndex.name[0] + 1;
  //Sort the list
...
function importCSVExcuteAll(){
  ...
  resetFile();
  list_all_files_inside_folder(folderGoogleDriveID, 
                               mainSheet, 
                               address_firstCell_A1_Style.csvFileList);
  
  returnListRangeExcludeTopRows(mainSheet, 
                                address_firstCell_A1_Style.csvFileList, 
                                1).applyRowBanding();//Banding rows for csvFileList
  ...
  returnListRangeExcludeTopRows(mainSheet, 
                                ...
                                1).applyRowBanding();//Banding rows for sheetList
