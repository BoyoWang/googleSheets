function updateSheetList() {
  ...
  var rangeCurrentSheetList = mainSheet.getRange(address_firstCell_A1_Style.sheetList).getDataRegion();
...
function changeSheetsName(){
  ...
  var sheetNewNameIndex = address_firstCell_A1_Style.sheetListTitleColIndex.newName[0];
  
  var sheetCurrentNameArray = [];
  var sheetNewNameArray = [];
  for(var i = 0; i < rangeSheetListArray.length; i++){
    if ((rangeSheetListArray[i][sheetNewNameIndex] == "-") || (rangeSheetListArray[i][sheetNewNameIndex] == "")){
  ...
  for(var i = 0; i < rangeSheetListArray.length; i++){
    spreadsheet.getSheetByName(sheetCurrentNameArray[i]).setName(sheetNewNameArray[i]);
  };
