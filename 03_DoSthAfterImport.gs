function ApplyActionToAllSheets() {
  ...
  //Aquire sheet list
  var rangesheetList = returnListRangeExcludeTopRows(mainSheet, address_firstCell_A1_Style.sheetList, 2);  
  ...
  for (i = 0; i < sheetList.length; i++){
    ...
    //copy needed columns
    var columnsToKeepText = [["us", 'General'], 
                             ["CH1", "#,##0.000"]];
...
function CreateAndAdjustTimeColumn(sheet) {
  ...
  spreadsheet.getRange('A3:A4').autoFillToNeighbor(SpreadsheetApp.AutoFillSeries.DEFAULT_SERIES);
