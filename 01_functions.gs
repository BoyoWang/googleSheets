...
function makeFirst2ArrayOfLists(/*object*/ListTitleColIndexObject,/*string*/listMainTitle){
  var indexInfoArray = changeObjectValueToArray(ListTitleColIndexObject);
  indexInfoArray.sort(function(a,b) {return a[0] - b[0];});//sort the indexInfoArray by index 
...
function returnListRangeExcludeTopRows(sheet, firstCellAddress_In_A1Style, intExcludeRowNum){
  var spreadsheet = SpreadsheetApp.getActive();
//  var sheet = spreadsheet.getActiveSheet();  
  var dataRegion = sheet.getRange(firstCellAddress_In_A1Style) .getDataRegion();  
  var rangeToReturn = sheet.getRange((dataRegion.getRow() + intExcludeRowNum), 
